...
namespace {
bool is_sssv_trace_ucode_enabled() {
    static int cached_enabled = -1;
    if (cached_enabled < 0) {
        const char* env_value = std::getenv("SSSV_TRACE_UCODE");
        cached_enabled = ((env_value != nullptr) && (env_value[0] == '1')) ? 1 : 0;
    }
...
const char* unknown_gbi_fallback_name(UnknownGBIFallback fallback) {
    switch (fallback) {
...
private:
    ...
    void maybe_apply_unknown_ucode_fallback(const OSTask* task) {
        ...
        if (is_sssv_trace_ucode_enabled()) {
            fprintf(stderr,
                "[SSSV][UCODE][TASK_FALLBACK] task_ucode=0x%08X task_ucode_data=0x%08X read_name=\"%s\" chosen=%s active_before=%s\n",
                task->t.ucode & 0x3FFFFFF,
                task->t.ucode_data & 0x3FFFFFF,
                ucode_name.c_str(),
                unknown_gbi_fallback_name(fallback),
                unknown_gbi_fallback_name(active_fallback));
        }
        ...
        const char* fallback_name = unknown_gbi_fallback_name(fallback);
        if (ucode_name.empty()) {
